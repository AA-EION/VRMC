...
describe("DEVICE_POSE", () => {
  it("round-trips a placement", () => {
    const body = roundTrip(PacketKind.DEVICE_POSE, (w) =>
      writeDevicePose(w, PLACED),
    );
  ...
  it("resolves a placement more finely than a hand can be measured", () => {
     ...
     * could ever produce.
     */
    const fine: DevicePlacement = {
      ...PLACED,
      centre: [1.234567, -2.345678, 9.876543],
    };
    const body = roundTrip(PacketKind.DEVICE_POSE, (w) =>
      writeDevicePose(w, fine),
    );
    const back = readDevicePose(body)!;
    for (let i = 0; i < 3; i++) {
  ...
  it("rejects numbers a room cannot contain", () => {
    ...
    expect(isPlausiblePlacement({ ...PLACED, centre: [NaN, 0, 0] })).toBe(
      false,
    );
    expect(isPlausiblePlacement({ ...PLACED, centre: [0, 1e9, 0] })).toBe(
      false,
    );
...
describe("layouts", () => {
  ...
  it("round-trips a saved arrangement", () => {
    const body = roundTrip(PacketKind.LAYOUT_SAVE, (w) =>
      writeLayoutSave(w, layout),
    );
  ...
  it("stores the model beside each placement", () => {
    ...
    const back = readLayoutSave(body)!;
    expect(back.entries.map((e) => e.model)).toEqual([
      "launchpad-x",
      "launchpad-pro-mk3",
    ]);
  });
  ...
  it("round-trips the whole set and which one is current", () => {
    const body = roundTrip(PacketKind.LAYOUT_STATE, (w) =>
      writeLayoutState(w, {
        layouts: [layout, { name: "Couch", entries: [] }],
        current: "Couch",
      }),
    );
  ...
  it("sends what fits rather than nothing when the list is too big", () => {
    /*
     * A packet body is 4080 bytes and `MAX_LAYOUTS` never bounded this: what
     * costs bytes is devices and name lengths, not the number of layouts.
     * Sixteen full layouts of eight devices is about 5 KB.
     *
     * The old code returned false at that point and `Broadcaster.sendLayouts`
     * returned without sending, so the headset got no layout state at all —
     * including `current`, the one field it needs to restore the arrangement
     * somebody is standing in. Losing the oldest few is a real loss; losing
     * every one of them silently is a worse one.
     */
    const big = {
      layouts: Array.from({ length: MAX_LAYOUTS }, (_, i) => ({
        name: `Layout ${i}`.padEnd(MAX_LAYOUT_NAME_BYTES, "-"),
    ...
    // The arithmetic, on its own: it does not all fit.
    expect(fitLayouts(big, MAX_LAYOUT_BODY_BYTES).length).toBeLessThan(
      big.layouts.length,
    );
    expect(fitLayouts(big, MAX_LAYOUT_BODY_BYTES).length).toBeGreaterThan(0);

    const w = new PacketWriter();
    w.begin(PacketKind.LAYOUT_STATE);
    const written = writeLayoutState(w, big);

    // The whole point: it succeeds, having written fewer than it was offered.
    // The old code returned a failure here and the caller sent nothing.
    expect(written).toBeGreaterThan(0);
    expect(written).toBeLessThan(MAX_LAYOUTS);

    const frame = w.finish(0).slice();
    const r = new PacketReader();
    expect(r.read(frame, null)).toBe(0);
    const back = readLayoutState(r.bodyView());

    // Readable, rather than a body that claims more layouts than it carries.
    expect(back).not.toBeNull();
    expect(back!.layouts).toHaveLength(written);
    // Whatever else was lost, the name of the current arrangement survived.
    expect(back!.current).toBe("Layout 0");
    // And the ones that did fit are intact, not truncated mid-entry.
    for (const l of back!.layouts) expect(l.entries).toHaveLength(8);
  });

  it("reports how many it wrote, so a caller can say what was dropped", () => {
    const w = new PacketWriter();
    w.begin(PacketKind.LAYOUT_STATE);
    expect(writeLayoutState(w, { layouts: [layout], current: "Studio" })).toBe(
      1,
    );
  });
  ...
  it("round-trips a bare name for apply and delete", () => {
    const body = roundTrip(PacketKind.LAYOUT_APPLY, (w) =>
      writeLayoutName(w, "Couch"),
    );
...
describe("LINK_STATS", () => {
  ...
  it("round-trips what the bridge measures", () => {
    const body = roundTrip(PacketKind.LINK_STATS, (w) =>
      writeLinkStats(w, quality),
    );
